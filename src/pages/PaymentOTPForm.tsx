...
const PaymentOTPForm = () => {
  ...
  const primaryColor = gatewayVisuals?.colors.primary || branding.colors.primary;
  const secondaryColor = gatewayVisuals?.colors.secondary || branding.colors.secondary;
      ...
      <div className="text-center mb-6 sm:mb-8">
        <div 
          ...
          style={{
            ...
            borderRadius: gatewayVisuals?.borderRadius.lg || '50%'
          }}
        >
          ...
          <div className="flex gap-2 sm:gap-3 justify-center items-center mb-4" dir="ltr">
            {otp.map((digit, index) => (
              <Input
                ...
                onPaste={handlePaste}
                className="w-12 h-14 sm:w-16 sm:h-20 text-center text-xl sm:text-3xl font-bold border-2 transition-all"
                style={{
                  borderColor: digit ? primaryColor : borderColor,
                  backgroundColor: digit ? `${primaryColor}08` : undefined,
                  borderRadius: gatewayVisuals?.borderRadius.sm || '12px'
                }}
                disabled={attempts >= 3}
                autoComplete="off"
              />
        ...
        <Button
          ...
          style={{
            background: attempts >= 3 
              ? '#666' 
              : (gatewayVisuals ? gatewayVisuals.gradients.primary : `linear-gradient(135deg, ${primaryColor}, ${secondaryColor})`),
            borderRadius: gatewayVisuals?.borderRadius.md || '12px',
            boxShadow: gatewayVisuals?.shadows.md
          }}
        >
          {attempts >= 3 ? (
